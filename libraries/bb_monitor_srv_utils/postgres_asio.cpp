...
namespace postgres_asio {
  ...
  connection_pool::connection_pool(boost::asio::io_service& fg, boost::asio::io_service& bg)
  : _fg_ios(fg)
  , _bg_ios(bg) {
  }
  ...
  // TBD
  void connection_pool::release(std::shared_ptr<postgres_asio::connection>) {
  }
}
