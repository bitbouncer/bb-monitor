...
class put_logs_call_data : public call_data {

public:
  // Take in the "service" instance (in this case representing an asynchronous
  // server) and the completion queue "cq" used for asynchronous communication
  // with the gRPC runtime.std::shared_ptr<kafka_sink_factory>
  put_logs_call_data(bb_monitor::MonitorSink::AsyncService* service, grpc::ServerCompletionQueue* cq, std::shared_ptr<kafka_sink_factory> factory)
      : service_(service)
      , cq_(cq)
      , stream_(&ctx_)
      , status_(START)
    ...
    status_ = START;
    service_->RequestPutLogs(&ctx_, &stream_, cq_, cq_, this);
  }

  ~put_logs_call_data(){
    if (_sink)
      LOG(INFO) << "log rpc done, client: " << _sink->client_id() << ", source: " << _source_ip << ", nr_of_msg: " << _msg_count;
  }
...
private:
  ...
  ::grpc::ServerAsyncReader<bb_monitor::PutLogsResponse, bb_monitor::LogBundle> stream_;
  CallStatus status_;  // The current serving state.
  std::shared_ptr<kafka_sink_factory> _sink_factory;
  std::shared_ptr<bb::bb_kafka_sink<std::string, bb_avro_logline>> _sink;
};