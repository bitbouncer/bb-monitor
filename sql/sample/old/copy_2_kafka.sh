#!/bin/bash
export TOPIC_NAME=BB_MONITOR_AUTH_VIEWv2
...
#postgres2kafka --db_host=localhost --db_dbname=bb_monitor --db_user=postgres --db_password=docker --table=auth_view --id_column=id --timestamp_column=ts --topic=$TOPIC_NAME --oneshot
